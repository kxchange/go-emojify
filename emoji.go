...
// from 
// https://github.com/urakozz/go-emoji/blob/master/parser.go#L11
...
var Unreg = regexp.MustCompile(emojiiPattern)

// Unemoji() returns code for the given emoji. If the
// specified emoji does not exist, Unemoji() returns the emoji
func Unemoji(emoji string) string {
	
	r, _ := utf8.DecodeRuneInString(emoji)
	hexKey, err := unquoteCodePoint(fmt.Sprintf(`%X`, r))	

	if err != nil {
		return emoji
	}

	val, ok := emojiRevCodeMap[hexKey]
	if !ok {
		return emoji
	}
...
func Unemojify(emojis string) string {
	return Unreg.ReplaceAllStringFunc(emojis, func(s string) string {
		return Unemoji(s)
	})
}

// Emoji() returns emoji for the given emoji. If the
// specified emoji does not exist, Emoji() returns the assigned emoji
...
type EmojiFunc func(name string) string

func EmojifyFunc(emojis string, action EmojiFunc) string {
	return reg.ReplaceAllStringFunc(emojis, func(str string) string {
		w := action(str)
		return  w+ ""
	})
